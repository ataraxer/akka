...
class FlowSplitWhenSpec extends AkkaSpec {
  ...
  class SubstreamsSupport(splitWhen: Int = 3, elementCount: Int = 6, eagerCancel: Boolean = false) {
    val source = Source(1 to elementCount)
    val groupStream = source.splitWhen(eagerCancel)(_ == splitWhen).lift.runWith(Sink.asPublisher(false))
  ...
  "splitWhen" must {
    ...
    "support eager cancellation of master stream on cancelling substreams" in assertAllStagesStopped {
      new SubstreamsSupport(splitWhen = 5, elementCount = 8, eagerCancel = true) {
        val s1 = StreamPuppet(getSubFlow().runWith(Sink.asPublisher(false)))
        s1.cancel()
        masterSubscriber.expectComplete()
      }
