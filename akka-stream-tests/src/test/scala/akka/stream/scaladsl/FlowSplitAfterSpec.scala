...
class FlowSplitAfterSpec extends AkkaSpec {
  ...
  class SubstreamsSupport(splitAfter: Int = 3, elementCount: Int = 6, eagerCancel: Boolean = false) {
    val source = Source(1 to elementCount)
    val groupStream = source.splitAfter(eagerCancel)(_ == splitAfter).lift.runWith(Sink.asPublisher(false))
  ...
  "splitAfter" must {
    ...
    "support eager cancellation of master stream on cancelling substreams" in assertAllStagesStopped {
      new SubstreamsSupport(splitAfter = 5, elementCount = 8, eagerCancel = true) {
        val s1 = StreamPuppet(expectSubFlow().runWith(Sink.asPublisher(false)))
        s1.cancel()
        masterSubscriber.expectComplete()
      }
